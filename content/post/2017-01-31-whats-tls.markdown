---
title: "How are SSL certificates issued?"
...
more interesting stuff to say. So in this post we will dissect some SSL
certificates!
...
* `CN=mail.google.com` is the "common name" -- the main domain that this certificate is for 
* an **expiry date**: Apr 12 18:50:00 2017 GMT
* The `X509v3 Subject Alternative Name:` section has other domains that this
  certificate also works for. The mail.google.com certificate also works for
  inbox.google.com, which makes sense.
...
1. At the same time, you generate a **private key** for you certificate. You keep this very secret and safe and do not show it to anybody.
...
The last thing we are going to talk about is [certificate transparency](https://www.certificate-transparency.org/). This is a super
...
about how certificates work on the inside!
